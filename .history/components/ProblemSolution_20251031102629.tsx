  ...
  return (
    <section id="problems" className="py-16 md:py-32 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Results Stats - PROFESSIONAL STYLING */}
        <div className="mt-12 md:mt-20 bg-linear-to-r from-indigo-600 via-purple-600 to-indigo-600 rounded-3xl p-8 md:p-12 text-white shadow-2xl overflow-hidden">
          {/* Decorative elements */}
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10 md:mb-12">
              {results.map((result, index) => (
                <div 
                  key={index}
                  className="bg-white bg-opacity-10 hover:bg-opacity-20 rounded-2xl p-6 md:p-8 border border-white border-opacity-30 backdrop-blur-sm transition-all duration-300 text-center hover:shadow-lg"
                >
                  {/* Icon */}
                  <div className="w-12 h-12 md:w-14 md:h-14 bg-white bg-opacity-20 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4 hover:bg-opacity-30 transition-all">
                    <result.icon className="w-6 h-6 md:w-7 md:h-7 text-white" />
                  </div>
                  
                  {/* Big Number */}
                  <div className="text-3xl md:text-5xl font-black mb-2 md:mb-3 text-white">{result.number}</div>
                  
                  {/* Label */}
                  <div className="text-sm md:text-base font-bold mb-2 text-white">{result.label}</div>
                  
                  {/* Description */}
                  <div className="text-xs md:text-sm text-indigo-100 leading-relaxed">{result.description}</div>
                </div>
              ))}
            </div>

            {/* Extra Info */}
            <div className="border-t border-white border-opacity-20 pt-8 md:pt-10 text-center">
